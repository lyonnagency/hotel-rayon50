	...
	if ( isset($_GET['page']) && $_GET['page'] == 'functions.php' ) {
		...
		if ( isset($_REQUEST['action']) && 'save' == $_REQUEST['action'] ) {
			...
			if(isset($_REQUEST['pp_envato_personal_token']) && !empty($_REQUEST['pp_envato_personal_token']) && $_REQUEST['pp_envato_personal_token'] != '[ThemeGoods Activation]')
			{
				...
				{
					$is_verified_envato_purchase_code = false;
					delete_option("pp_verified_envato_hoteller", true);
				}
...
function hoteller_enqueue_admin_page_scripts() 
{
	...
	wp_enqueue_style('hoteller-functions', get_template_directory_uri().'/functions/css/functions.css', false, HOTELLER_THEMEVERSION, 'all');

	wp_enqueue_style('switchery', get_template_directory_uri().'/functions/css/switchery.css', false, HOTELLER_THEMEVERSION, 'all');
	wp_enqueue_style("fontawesome", get_template_directory_uri()."/css/font-awesome.min.css", false, HOTELLER_THEMEVERSION, "all");
	...
	wp_enqueue_script('switchery', get_template_directory_uri().'/functions/switchery.js', false, HOTELLER_THEMEVERSION);
	
	wp_register_script('hoteller-theme-script', get_template_directory_uri().'/functions/theme_script.js', false, HOTELLER_THEMEVERSION, true);
	$params = array(
...
{
	wp_enqueue_style("hoteller-reset-css", get_template_directory_uri()."/css/core/reset.css", false, "");
	wp_enqueue_style("hoteller-wordpress-css", get_template_directory_uri()."/css/core/wordpress.css", false, "");
	wp_enqueue_style("hoteller-screen", get_template_directory_uri().'/css/core/screen.css', false, "", "all");
	...
	{
		case 'leftalign':
			wp_enqueue_style("hoteller-leftalignmenu", get_template_directory_uri().'/css/menus/leftalignmenu.css', false, "", "all");
		break;
		
		case 'hammenufull':
			wp_enqueue_style("hoteller-hammenufull", get_template_directory_uri().'/css/menus/hammenufull.css', false, "", "all");
		break;
		
		case 'centeralogo':
			wp_enqueue_style("hoteller-centeralogo", get_template_directory_uri().'/css/menus/centeralogo.css', false, "", "all");
		break;
	}
	
	//Add Font Awesome Support
	wp_enqueue_style("fontawesome", get_template_directory_uri()."/css/font-awesome.min.css", false, "", "all");
	wp_enqueue_style("themify-icons", get_template_directory_uri()."/css/themify-icons.css", false, HOTELLER_THEMEVERSION, "all");
    ...
    if(!empty($tg_frame))
    {
    	wp_enqueue_style("tg_frame", get_template_directory_uri()."/css/core/frame.css", false, HOTELLER_THEMEVERSION, "all");
    }
	
	if(HOTELLER_THEMEDEMO)
    {
	   	wp_enqueue_style('tooltipster', get_stylesheet_directory_uri()."/css/tooltipster.css", false, "", "all");
	    wp_enqueue_style('hoteller-demo', get_stylesheet_directory_uri()."/css/core/demo.css", false, "", "all");
	}
	...
	wp_enqueue_script("waypoints", get_template_directory_uri()."/js/waypoints.min.js", false, HOTELLER_THEMEVERSION, true);
	wp_enqueue_script("tilt", get_template_directory_uri()."/js/tilt.jquery.js", false, HOTELLER_THEMEVERSION, true);
	wp_enqueue_script("stellar", get_template_directory_uri()."/js/jquery.stellar.min.js", false, HOTELLER_THEMEVERSION, true);
	wp_enqueue_script('modulobox', get_template_directory_uri()."/js/modulobox.js", array(), HOTELLER_THEMEVERSION, true );
	wp_enqueue_script("hoteller-custom-plugins", get_template_directory_uri()."/js/core/custom_plugins.js", false, HOTELLER_THEMEVERSION, true);
	wp_enqueue_script("hoteller-custom-script", get_template_directory_uri()."/js/core/custom.js", false, HOTELLER_THEMEVERSION, true);
	...
	{
		wp_enqueue_script("sticky-kit", get_template_directory_uri()."/js/jquery.sticky-kit.min.js", false, HOTELLER_THEMEVERSION, true);
	...
	{
		wp_enqueue_script("lazy", get_template_directory_uri()."/js/jquery.lazy.js", false, HOTELLER_THEMEVERSION, true);
		$custom_lazy_script = '
		jQuery(function( $ ) {
	...
	if(!empty($pp_photostream))
	{
		...
		wp_enqueue_script("modernizr", get_template_directory_uri()."/js/modernizr.js", false, HOTELLER_THEMEVERSION, true);
		wp_enqueue_script("gridrotator", get_template_directory_uri()."/js/jquery.gridrotator.js", false, HOTELLER_THEMEVERSION, true);
...
function hoteller_register_mobile_css() 
{
	...
	wp_enqueue_style('hoteller-script-responsive-css', get_template_directory_uri()."/css/core/responsive.css", false, "", "all");
		...
		<?php 
			foreach ($hoteller_options as $value) {
				...
				if($value['type'] == 'section')
				{
					...
					echo '<a id="pp_panel_'.strtolower($value['name']).'_a" href="#pp_panel_'.strtolower($value['name']).'" class="nav-tab '.esc_attr($active).'"><span class="dashicons  '.$value['icon'].'"></span>'.str_replace('-', ' ', $value['name']).'</a>';
				}
			}
		?>
		</h2>
...
if (isset($_GET['activated']) && $_GET['activated'] && is_admin() && current_user_can('manage_options')){
	...
	update_option('elementor_cpt_support', array('post', 'page', 'footer', 'header'));
	update_option('elementor_page_title_selector', '#page_caption');
